...
* **Literal** - a literal is a constant that is defined as part of the instruction, rather than in a separate **DC** statement.  Literals are used in a statement in place of where a symbolic name would be used.  They are proceeded by an equal sign.  For example, **AP COUNT,=P'1'** adds the literal **1** to the value in the **COUNT** symbolic reference.  While it may look similar to an **Immediate** operation, these operations are completetly different, and apply to different types of instructions.
